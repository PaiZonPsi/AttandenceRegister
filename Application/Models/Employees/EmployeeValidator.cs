...
public class EmployeeValidator : AbstractValidator<EmployeeModel>
{
    public EmployeeValidator()
    {
        ...
        RuleFor(model => model.FirstName)
            .NotNull()
            .WithMessage(ErrorMessages.NotNullMessage)
        ...
        RuleFor(model => model.Email).NotNull()
            .WithMessage(ErrorMessages.NotNullMessage)
            .EmailAddress()
            .WithMessage(ErrorMessages.EmailFormatMessage)
            .MaximumLength(maximumLength)
            .WithMessage(maxLengthErrorMessage);
        RuleFor(model => model.PhoneNumber).NotNull()
            .WithMessage(ErrorMessages.NotNullMessage)
            .Length(9)
            .WithMessage(ErrorMessages.PhoneNumberFormatMessage);
    }
}